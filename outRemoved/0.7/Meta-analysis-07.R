...
here <- gsub("/", "\\\\", here)
corr <- 0.7
dataPath <- paste(here, "outRemoved", "Data", sep = "\\")
resultPathPost <- paste(here, "outRemoved", str_c(corr), "Results\\Post", sep = "\\")
resultPathDur <- paste(here, "outRemoved", str_c(corr), "Results\\Dur", sep = "\\")
Params <- c("LF", "HF", "LFHF", "RMSSD", "SDNN", "BRS")
removedStudy <- c("Keute et al. (2021)", "Shen et al. (2021) Ex.1",
                  "Shen et al. (2021) Ex.2", "Borges et al. (2019)",
                  "Clancy et al. (2014)")
